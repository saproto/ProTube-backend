    ...
    <ContentField>
        <label class="text-gray-600 dark:text-white text-2xl absolute"> The current Queue - {{ queueData.duration }}</label>
            <div class="flex overflow-x-scroll pt-10 no-scrollbar">
                ...
                <div v-else class="flex flex-nowrap h-full">
                    <ul v-for="(video, index) in queueData.queue" :video="video" :index="index" :key="video.id" class="grid inline-block px-3 w-96 min-h-full" >
                        <li :style='{background: `url(${video.thumbnail.url})`}' style="background-repeat: no-repeat; background-size: cover; background-position: center center;" class="group cursor-pointer col-span-1 flex group flex-col text-center  border-proto_blue border-l-4 rounded-sm shadow"> <!--divide-y dark:divide-proto_green divide-gray-500-->
                            <div v-show="admin" @click="$emit('remove-from-queue', video)" class="group-hover:bg-opacity-60 flex-1 rounded-m border-t border-b border-r dark:border-gray-800 border-gray-400 flex flex-col px-8 py-4 bg-white dark:bg-true_gray-800 bg-opacity-80">
